        ...
        <nav className="flex-1 px-2 space-y-1 bg-white">
          {navigation.map((item) => (
            <Link
              ...
              to={item.href}
              className={`
                ${location.pathname === item.href ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-600 hover:bg-blue-50 hover:text-blue-700'}
                group flex items-center px-2 py-2 text-sm font-medium rounded-md transition-colors
              `}
            >
              <item.icon
                className={`
                  ${location.pathname === item.href ? 'text-white' : 'text-gray-400 group-hover:text-blue-500'}
